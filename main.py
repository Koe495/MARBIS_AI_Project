...
# Import chính xác từ file hand_gestures bạn cung cấp
...
# Cấu hình MediaPipe Hand Landmarker (API mới, nhanh hơn)
    ...
    try:
        ...
        while True:
            ...
            if cmd:
                ...
                shared_state.state.CURRENT_STATUS = "Listening..."
                last_gesture_ts = time.time()  # Reset timeout để nghe tiếp câu sau
...
print(">> Tay Phải: Chuột + Listen (Ngón Cái + Út)")
print(">> 2 Tay: Volume + Zoom")
    ...
    while True:
        ...
        if result.hand_landmarks:
            ...
            if user_left and user_right:
                ...
                # Gọi hàm process_zoom_mode từ file của bạn
                # (Logic file cũ: Zoom ưu tiên trước)
            ...
            # --- KỊCH BẢN B: CHỈ CÓ TAY PHẢI (CHUỘT & LISTEN) ---
                ...
                # Gọi hàm process_gestures từ file của bạn
            ...
            # QUAN TRỌNG: Không làm gì cả để tránh lỗi "Ghost Click" của file gesture cũ
            elif user_left:
